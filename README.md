...
DATABASE_URL=mysql+pymysql://root:Vijay_Root%40123%21@localhost:3306/task_manager
...
Task Access & Permissions Logic

Business Rules
	1.	Task Ownership
	•	Each task has:
	•	assigned_user_id → user responsible for completing the task
	•	created_by → user who created the task
	2.	User Permissions
	•	A logged-in user can:
	•	Edit / Delete tasks they created
	•	View (read-only) tasks assigned to them by other users
	•	A logged-in user cannot edit or delete tasks created by other users, even if those tasks are assigned to them.
